...
request.defaults.headers.common['X-Rundeck-Auth-Token'] = config.RUNDECK_ACCESS_TOKEN;

app.set('views', './views');
app.set('view engine', 'ejs');
app.get('/', (req, res) => {
	request.get(config.RUNDECK_API_BASE_URL + 'projects')
		.then((result) => {
...
app.get('/:projectName/show', (req, res) => {
	...
	let projects = [];
	Promise.all([request.get(config.RUNDECK_API_BASE_URL + 'projects'), 
				request.get(config.RUNDECK_API_BASE_URL + `project/${projectName}/jobs?scheduledFilter=true`)])
		.then(info => {
			projects = info[0].data;
			let scheduledJobs = [];
			if (info[1].status === 200) {
				scheduledJobs = info[1].data.map(scheduled => {
					return Promise.all([ request.get(config.RUNDECK_API_BASE_URL + 'job/' + scheduled.id + '/info'),
								request.get(config.RUNDECK_API_BASE_URL + 'job/' + scheduled.id + '?format=xml'),
						...
						.then((pmResult) => {
			            	...
			            	if (pmResult[1].data !== undefined) {
				            	...
				            	scheduled.schedule = scheduleExp.time[0].$.seconds + ' ' + scheduleExp.time[0].$.minute + ' ' + scheduleExp.time[0].$.hour + ' ' 
...
app.listen(config.HTTP_PORT, () => {
	console.log(`Server is starting on port ${config.HTTP_PORT}`);
})